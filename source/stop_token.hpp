...
public:
    ...
    {
        auto __oldState = __state_.fetch_sub(__token_ref_increment,
    ...
    {
        auto __oldState = __state_.fetch_sub(__source_ref_increment,
                                             std::memory_order_acq_rel);
    ...
    void __remove_callback(__stop_callback_base *__cb) noexcept
    {
        ...
        if (__signallingThread_ == std::this_thread::get_id()) {
            ...
            if (__cb->__isRemoved_ != nullptr) {
                // Currently inside the callback, let the __request_stop()
                // method know the object is about to be destructed and that it
                // should not try to access the object when the callback
                // returns.
            ...
            while (!__cb->__callbackFinishedExecuting_.load(
...
private:
    ...
    static bool __is_stop_requestable(std::uint64_t __state) noexcept
    {
        ...
        return __is_stop_requested(__state) ||
               (__state >= __source_ref_increment);
    }
...
public:
    ...
    template <typename _CB,
              std::enable_if_t<std::is_constructible_v<_Callback, _CB>, int> =
                  0>
