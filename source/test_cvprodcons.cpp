...
{
    std::cout << "*** start exampleProducerConsumer(prodSleep=" << prodSec
              << "s, consSleep=" << consSec << "s, "
              << "interrupt=" << interrupt << ")" << std::endl;
    auto prodSleep = std::chrono::duration<double, std::milli>{
        prodSec}; // duration producer creates new values
    auto consSleep = std::chrono::duration<double, std::milli>{
        consSec}; // duration producer deals with new values
    ...
    std::thread producer{[&] {
        auto nextValue = [val = 0]() mutable { return ++val; };

        // std::cout << "\nP: lock " << std::endl;
        std::unique_lock lock{itemsMx};
        while (!stoken.stop_requested()) {
            ////std::cout << "\nP: wait " << std::endl;
            if (!itemsCV.wait_until(
                    lock, [&] { return items.size() < maxQueueSize; },
                    stoken)) {
                // std::cout << "\nP: wait returned false " << std::endl;
                return;
            }
            //// OOPS: NOTE: true does NOT necessarily meant that we have no
            /// interrupt !!! /std::cout << "\nP: wait returned true " <<
            /// std::endl;
    ...
    std::thread consumer{[&] {
        ...
        for (;;) {
            ...
            // std::cout << "\nC: wait " << std::endl;
            if (!itemsCV.wait_until(
                    lock, [&] { return !items.empty(); }, stoken)) {
            ...
            for (int item : items) {
                ...
                if (item == 42) {
                    ...
                    // Whoops, this is being called while holding a lock on
                    // mutex 'itemMx'!
                    strm << " INTERRUPT";
