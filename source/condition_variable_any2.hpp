...
class condition_variable_any2
{
    template <typename Lockable> struct unlock_guard
    {
        unlock_guard(Lockable &mtx_) : mtx(mtx_) { mtx.unlock(); }
        ~unlock_guard() { mtx.lock(); }
...
public:
    ...
    condition_variable_any2() : internals{std::make_shared<cv_internals>()} {}
    ~condition_variable_any2() {}
    condition_variable_any2(const condition_variable_any2 &) = delete;
    condition_variable_any2 &
    operator=(const condition_variable_any2 &) = delete;

    void notify_one() noexcept { internals->notify_one(); }
    void notify_all() noexcept { internals->notify_all(); }
    ...
    void wait(Lockable &lock, Predicate pred)
    {
        ...
        // reacquired before calling pred(). (otherwise the test_cvrace_pred
        // test case fails)
        auto local_internals = internals;
        while (!pred()) {
            std::unique_lock<std::mutex> first_internal_lock(
                local_internals->m);
            unlock_guard<Lockable> unlocker(lock);
            std::unique_lock<std::mutex> second_internal_lock(
                std::move(first_internal_lock));
                    ...
                    Predicate pred)
    {
        ...
        while (!pred()) {
            bool shouldStop;
            {
                ...
                std::unique_lock<std::mutex> second_internal_lock(
                    std::move(first_internal_lock));
                shouldStop = (local_internals->cv.wait_until(
                                  second_internal_lock, abs_time) ==
    ...
    bool wait_for(Lockable &lock, const chrono::duration<Rep, Period> &rel_time,
                  Predicate pred)
    {
        return wait_until(lock, std::chrono::steady_clock::now() + rel_time,
                          std::move(pred));
    ...
    template <class Lockable, class Predicate>
    bool wait_until(Lockable &lock, Predicate pred, stop_token stoken);
...
private:
    ...
    //   all threads shall have been notified;
    //             they may subsequently block on the lock specified in the
    //             wait. This relaxes the usual rules, which would have required
    //             all wait calls to happen before destruction. Only the
    //             notification to unblock the wait needs to happen before
    //             destruction. The user should take care to ensure that no
    //             threads wait on *this once the destructor has been started,
    //             especially when the waiting threads are calling the wait
    //             functions in a loop or using the overloads of wait, wait_for,
    ...
    // functions wait (wait_for, etc.). (libc++'s implementation gets this
    // right:
    ...
    //  directly, and the wait functions create a local shared_ptr<mutex> copy
    //  on entry so that if *this destructs out from under the thread executing
    //  the wait function, the mutex stays alive until the wait function
    //  returns.)
};
...
// - false otherwise (i.e. on interrupt)
template <class Lockable, class Predicate>
inline bool condition_variable_any2::wait_until(Lockable &lock, Predicate pred,
                                                stop_token stoken)
{
    ...
    stop_callback cb(stoken,
                     [&local_internals] { local_internals->notify_all(); });
    while (!pred()) {
        std::unique_lock<std::mutex> first_internal_lock(local_internals->m);
        if (stoken.stop_requested()) {
            // pred() has already evaluated to 'false' since we last a acquired
            // 'lock'
            return false;
        }
        unlock_guard<Lockable> unlocker(lock);
...
template <class Lockable, class Rep, class Period, class Predicate>
inline bool
condition_variable_any2::wait_for(Lockable &lock,
                                  const chrono::duration<Rep, Period> &rel_time,
                                  Predicate pred, stop_token stoken)
{
    auto abs_time = std::chrono::steady_clock::now() + rel_time;
    return wait_until(lock, abs_time, std::move(pred), std::move(stoken));
}
