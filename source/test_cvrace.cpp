...
void testCVDeadlock()
{
...

    {
        std::jthread t1([&ready, &readyMutex, &readyCV](std::stop_token it) {
            std::cout << "\n"
                      << std::this_thread::get_id() << ": t1: lock "
                      << &readyMutex << std::endl;
            std::unique_lock<std::mutex> lg{readyMutex};
            std::cout << "\n"
                      << std::this_thread::get_id() << ": t1: wait"
                      << std::endl;
            readyCV.wait_until(
                lg, [&ready] { return ready; }, it);
            if (it.stop_requested()) {
                std::cout << "\n"
                          << std::this_thread::get_id() << ": t1: signal stop"
                          << std::endl;
            } else {
                std::cout << "\n"
                          << std::this_thread::get_id() << ": t1: ready"
                          << std::endl;
            }
        });
        ...
        std::jthread t2([&ready, &readyMutex, &readyCV, &t1StopSource](
                            std::stop_token /*t2_stop_token_not_used*/) {
            std::cout << "\n"
                      << std::this_thread::get_id() << ": t2: lock "
                      << &readyMutex << std::endl;
            std::unique_lock<std::mutex> lg{readyMutex};
            std::cout << "\n"
                      << std::this_thread::get_id() << ": t2: signal stop"
                      << std::endl;
            t1StopSource.request_stop();
            std::cout << "\n"
                      << std::this_thread::get_id() << ": t2: signal-stop done"
                      << std::endl;
        });
