...
//  implementation of jthread. However this is such a complex subject that no
...
//     subsequently block on the lock specified in the wait.
//            This relaxes the usual rules, which would have required all wait
//            calls to happen before destruction. Only the notification to
//            unblock the wait needs to happen before destruction. The user
//            should take care to ensure that no threads wait on *this once the
//            destructor has been started, especially when the waiting threads
//            are calling the wait functions in a loop or using the overloads of
...
// destructed mutex mut. To fix this, there must be shared ownership of the data
// member mut between the condition_variable_any object and the member functions
// wait (wait_for, etc.).
//
...
// entry so that if *this destructs out from under the thread executing the wait
// function, the mutex stays alive until the wait function returns.
//
...
//  your condition_variable_any2 with an internal mutex that checks for
//  unlock-after-destruction. And the problem with that is now you no no longer
//  have a std::mutex to put into your internal std::condition_variable...
//
