...
template <typename Duration> std::string asString(const Duration &dur)
{
    auto durMilliseconds =
...
void interruptByDestructor()
{
    ...
    bool t1WasInterrupted = false;
    {
        ...
        std::jthread t1([interval, &t1WasInterrupted](std::stop_token stoken) {
            printID("t1 STARTED with interval " + asString(interval) +
                    " with id");
        ...
        // call destructor after 4 times the interval (should signal the
        // interrupt)
        std::this_thread::sleep_for(4 * interval);
        assert(!t1.get_stop_source().stop_requested());
        std::cout << "\n- destruct jthread t1 (should signal interrupt)"
                  << std::endl;
    }
...
{
    std::cout << "\n*** start interruptStartedThreadWithSubthread(): "
              << std::endl;
    auto interval = 0.2s;
    {
        ...
        std::this_thread::sleep_for(4 * interval);
        std::cout << "\n- interrupt jthread t1 (should signal interrupt to t2)"
                  << std::endl;
        t1.request_stop();
        assert(t1.get_stop_source().stop_requested());
        std::cout << "\n- join jthread t1" << std::endl;
...
void foo(const std::string &msg) { printID(msg); }
        ...
        std::jthread t1([&itPtr](std::stop_token sToken) {
            ...
            try {
                ...
                for (int i = 0; numInterrupts < 2 && i < 500; ++i) {
                    ...
                    // print character according to current interrupt token
                    // state:
...
void testConcurrentInterrupt()
{
    ...
    std::stop_source is;
    {
        // XXX std::atomic<int> numInterrupts{0};
        std::cout << "\n- start jthread t1" << std::endl;
        ...
        std::cout << "\n- loop over " << numThreads
                  << " threads that request_stop() concurrently" << std::endl;
        ...
        std::cout << "\n- requestStopNumTrue: " << requestStopNumTrue
                  << std::endl;
...
{
    std::cout << "\n*** start testEnableIfForCopyConstructor_CompileTimeOnly()"
              << std::endl;
    {
        std::jthread t1;
