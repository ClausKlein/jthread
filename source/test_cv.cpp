...
void cvIWait(std::stop_token sToken, int id, bool &ready,
             std::mutex &readyMutex, std::condition_variable_any2 &readyCV,
             bool notifyCalled)
{
...
void testStdCV(bool callNotify)
{
    ...
    std::cout << "*** start testStdCV(callNotify=" << callNotify << ")"
              << std::endl;
    ...
    {
        std::jthread t1(
            [&ready, &readyMutex, &readyCV, callNotify](std::stop_token it) {
                {
                    std::unique_lock lg{readyMutex};
...
{
    // test the basic jthread API
    std::cout << "*** start testCVPred(callNotify=" << callNotify << ")"
              << std::endl;
...

    {
        std::jthread t1([&ready, &readyMutex, &readyCV,
                         callNotify](std::stop_token it) {
            try {
                std::unique_lock lg{readyMutex};
                // readyCV.iwait(lg, [&ready] { return ready; });
                readyCV.wait_until(
                    lg, [&ready] { return ready; }, it);
                if (it.stop_requested()) {
                    throw "interrupted";
                }
                assert(callNotify);
...
{
    // test the basic jthread API
    std::cout << "*** start testCVStdThreadNoPred(callNotify=" << callNotify
              << ")" << std::endl;
    ...
    {
        std::thread t1(
            [&ready, &readyMutex, &readyCV, it = is.get_token(), callNotify] {
                {
                    std::unique_lock lg{readyMutex};
                    bool ret = readyCV.wait_until(
                        lg, [&ready] { return ready; }, it);
                    if (ret) {
                        std::cout << "t1: ready" << std::endl;
...
{
    // test the basic jthread API
    std::cout << "*** start testCVStdThreadPred(callNotify=" << callNotify
              << ")" << std::endl;
...
void testMinimalWait(int sec)
{
    ...
    try {
        ...
        std::condition_variable_any2 readyCV;
        {
            ...
            std::cout
                << "- leave scope (should signal interrupt and unblock CV wait)"
                << std::endl;
        } // leave scope of t1 without join() or detach() (signals cancellation)
        std::cout << "\n*** OK" << std::endl;
    } catch (const std::exception &e) {
...
void testMinimalWaitFor(int sec1, int sec2)
{
    ...
    try {
        ...
        std::condition_variable_any2 readyCV;
        {
            std::jthread t1([&ready, &readyMutex, &readyCV, durInt,
                             durWait](std::stop_token it) {
                try {
                    std::cout << "\n- start t1" << std::endl;
                    auto t0 = std::chrono::steady_clock::now();
                    {
                        std::unique_lock lg{readyMutex};
                        readyCV.wait_for(
                            lg, durWait, [&ready] { return ready; }, it);
                    }
...
void testTimedCV(bool callNotify, bool callInterrupt, Dur dur)
{
    ...
    std::cout << "*** start testTimedCV(callNotify=" << callNotify << ", "
              ...
              << std::chrono::duration<double, std::ratio<1>>(dur).count()
              << "s)" << std::endl;
...

    {
            ...
            while (timesDone < 3) {
                {
                    ...
                    auto ret = readyCV.wait_for(
                        lg, dur, [&ready] { return ready; }, it);
                    if (dur > 5s) {
        ...
        std::this_thread::sleep_for(1.5s);
        std::cout << "- leave scope (should at latest signal interrupt)"
                  << std::endl;
    } // leave scope of t1 without join() or detach() (signals cancellation)
    std::cout << "\n*** OK" << std::endl;
}
...
template <int numExtraCV> void testManyCV(bool callNotify, bool callInterrupt)
{
...

    {
        ...
        std::array<bool, numExtraCV> arrReady{};
        ...
        std::jthread t0(cvIWait, 0, std::ref(ready), std::ref(readyMutex),
                        std::ref(readyCV), callNotify);
        {
            ...
            // token:
            std::cout << "\n- loop to start " << numExtraCV
                      << " threads sharing the token and waiting concurently"
                      << std::endl;

            std::vector<std::jthread> vThreads;
            for (int idx = 0; idx < numExtraCV; ++idx) {
                std::this_thread::sleep_for(0.1ms);
                std::jthread t([idx, t0stoken = t0ssource.get_token(),
                                &arrReady, &arrReadyMutex, &arrReadyCV,
                                callNotify] {
                    // use interrupt token of t0 instead
                    // NOTE: disables signaling interrupts directly to the
                    // thread
                    cvIWait(t0stoken, idx + 1, arrReady[idx],
                            arrReadyMutex[idx], arrReadyCV[idx], callNotify);
                });
            ...
            if (callNotify) {
                std::cout << "\n- set predicate and call notify on t0"
                          << std::endl;
                {
                    std::lock_guard lg(readyMutex);
                    ready = true;
                ...
                std::cout << "\n- call notify_one() on other threads"
                          << std::endl;
                for (int idx = 0; idx < numExtraCV; ++idx) {
                    {
        ...
        } // leave scope of additional threads (already notified/interrupted or
          // detached)
...
int main()
{
    try {
        ...
        std::cout << "\n\n**************************\n";
        testTimedCV(true, false,
                    200ms); // call notify(), don't call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedCV(false, true,
                    200ms); // don't call notify, call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedCV(false, false,
                    200ms); // don't call notify, don't call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedCV(true, false,
                    60s); // call notify(), don't call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedCV(false, true, 60s); // don't call notify, call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedCV(false, false,
                    60s); // don't call notify, don't call request_stop()

        std::cout << "\n\n**************************\n";
        testTimedIWait(true, false,
                       200ms); // call notify(), don't call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedIWait(false, true,
                       200ms); // don't call notify, call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedIWait(false, false,
                       200ms); // don't call notify, don't call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedIWait(true, false,
                       60s); // call notify(), don't call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedIWait(false, true,
                       60s); // don't call notify, call request_stop()
        std::cout << "\n\n**************************\n";
        testTimedIWait(false, false,
                       60s); // don't call notify, don't call request_stop()
        ...
        std::cout << "\n\n**************************\n";
        testManyCV<9>(false, false); // don't call notify, don't call
                                     // request_stop() (implicit interrupt)
    } catch (const std::exception &e) {
