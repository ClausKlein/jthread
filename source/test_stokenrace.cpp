...
void testCallbackConcUnregister()
{
    ...
    optCB.emplace(
        stok,
        std::ref(cb1)); // ref() necessary because we copy to std::function
    // anabling this thead create a CORE DUMP at the end of cb1 being in
    // request_stop():
    std::thread t1{[&] {
        while (!cb1end) {
            if (!cb1called) {
...
void testCallbackConcUnregisterOtherThread()
{
    ...
    // enabling this thead create a CORE DUMP at the end of cb1 being in
    // request_stop():
    std::thread t1{[&] {
        while (!cb1end) {
            if (!cb1called) {
