...
TEST(ConcurrentCallbackRegistration)
{
    auto threadLoop = [](std::stop_token token) {
        ...
        while (!cancelled) {
            std::stop_callback registration{token, [&] { cancelled = true; }};
...
TEST(CallbackDeregistrationDoesNotWaitForOtherCallbacksToFinishExecuting)
{
    ...
    std::optional<std::stop_callback<decltype(dummyCallback) &>> cb1{
        std::in_place, src.get_token(), dummyCallback};
    ...
    std::optional<std::stop_callback<decltype(dummyCallback) &>> cb2{
        std::in_place, src.get_token(), dummyCallback};

    std::thread signallingThread{[&] { src.request_stop(); }};

    // Wait until the callback starts executing on the signalling-thread.
    // The signalling thread will remain blocked in this callback until we
    // release it.
    {
        std::unique_lock lock{mut};
    ...
    // Finally, signal the callback to unblock and wait for the signalling
    // thread to finish.
    {
        std::unique_lock lock{mut};
...
template <typename CB> struct callback_batch
{
    using callback_t = std::stop_callback<CB &>;

    callback_batch(std::stop_token t, CB &callback)
        : r0(t, callback), r1(t, callback), r2(t, callback), r3(t, callback),
          r4(t, callback), r5(t, callback), r6(t, callback), r7(t, callback),
          r8(t, callback), r9(t, callback)
    {}
